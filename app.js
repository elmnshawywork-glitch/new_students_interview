...
RENDER.interview = async function(){
  ...
  var students=b.students.slice().sort(function(a,c){
    var ea=(a.evaluatorsCount||0)>0?1:0, ec=(c.evaluatorsCount||0)>0?1:0;
    if(ea!==ec) return ea-ec;                       // غير المُقيَّمين أولاً
  ...
  var pending=students.filter(function(s){return !(s.evaluatorsCount>0);}).length;

  var html='<div class="card"><h3>تقييم المقابلة</h3>'+
    '<p class="card-sub">قيّم سلوك الطالب المنتقل من الإعدادية إلى التعليم الفني (كل مؤشر من '+max+'). المقابلة تمثّل '+esc(cfg.weightInterview)+'% من الدرجة. <b>'+pending+'</b> طالب في انتظار التقييم.</p>'+
    '<div class="field"><label>اختر الطالب (غير المُقيَّمين أولاً)</label><select id="iv-stu"><option value="">— اختر —</option>'+
    students.map(function(s){var done=(s.evaluatorsCount||0)>0; return '<option value="'+esc(s.nationalId)+'">'+(done?"✔ ":"• ")+esc(s.name)+' — '+esc(s.nationalId)+(done?" (مُقيَّم)":"")+'</option>';}).join("")+'</select></div>'+
    '<div id="iv-head"></div>'+
