...
var Demo = (function () {
  ...
  function computeResult(nid){
    ...
    if(ivs.length){ var s=0; ivs.forEach(function(v){ s+=(Number(v.avg)/iMax)*100; }); interviewPct=s/ivs.length; }
    ...
    switch(b.action){
      ...
      case "importStudents": {
        ...
        (b.rows||[]).forEach(function(s){
          ...
          var ex=db.students.filter(function(x){return String(x.nationalId)===String(s.nationalId);})[0];
          if(ex){ Object.assign(ex,s); updated++; } else { db.students.push(Object.assign({createdAt:Date.now()},s)); added++; }
        });
