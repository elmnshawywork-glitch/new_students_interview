...
function doPost(e) {
  ...
  var WRITES = { setConfig:1, createUser:1, deleteUser:1, registerStudent:1, importStudents:1, setUnitGrade:1, importUnitGrades:1, gradeExam:1, submitLang:1, submitInterview:1, resetLangExam:1, regradeAll:1 };
  ...
  try {
    switch (action) {
      ...
      case 'studentBegin':    return json_(studentBegin_(req));
      case 'submitLang':      return json_(submitLang_(req));
      case 'getPassages':     return json_({ ok: true, passages: getPassages_() });
      case 'regradeAll':      return json_(regradeAll_(req));
...
/* ============================================================
 *  مسار الامتحان القابل للتوسّع (عدد كبير متزامن)
 *  - studentBegin: نداء تحقّق واحد خفيف (هل الطالب مسجّل/سبق الأداء)
 *  - submitLang: كتابة إضافة واحدة فقط (بدون قراءة/ذكاء اصطناعي) — سريعة
 *  - regradeAll: إعادة التصحيح بالذكاء الاصطناعي دفعة واحدة (يشغّلها المدير لاحقًا)
 * ============================================================ */
function studentBegin_(req) {
  var nid = String(req.nationalId || '').replace(/[^\d]/g, '');
  var s = readAll_('Students').filter(function (x) { return String(x.nationalId) === nid; })[0];
  if (!s) return { ok: false, error: 'الطالب غير مسجّل' };
  var done = readAll_('LangResults').some(function (x) { return String(x.nationalId) === nid; });
  return { ok: true, student: { nationalId: nid, name: s.name, school: s.school }, alreadyDone: done };
}

// تسليم الامتحان: صف واحد بالإضافة، مع درجات محسوبة في المتصفح (سريع جدًا وقابل للتوسّع)
function submitLang_(req) {
  var nid = String(req.nationalId || '').replace(/[^\d]/g, '');
  var sc = req.scores || {};
  var answers = req.answers || {};
  var row = [nid, req.enListenId || '', Number(sc.enListen) || 0, req.translateId || '', Number(sc.translate) || 0,
             req.arListenId || '', Number(sc.arListen) || 0, Number(sc.langTotal) || 0, req.feedback || 'auto',
             JSON.stringify(answers), new Date()];
  sheet_('LangResults').appendRow(row);      // إضافة فقط — بدون قراءة
  return { ok: true };
}

// إعادة تصحيح كل الامتحانات بالذكاء الاصطناعي (اختياري، دفعة واحدة)
function regradeAll_(req) {
  requireAdmin_(req);
  var key = PropertiesService.getScriptProperties().getProperty('ANTHROPIC_API_KEY');
  if (!key) return { ok: false, error: 'لا يوجد مفتاح ذكاء اصطناعي (ANTHROPIC_API_KEY)' };
  var lang = readAll_('LangResults');
  var sh = sheet_('LangResults');
  var updated = 0;
  for (var i = 0; i < lang.length; i++) {
    var lr = lang[i];
    var a = {}; try { a = JSON.parse(lr.answers || '{}'); } catch (e) { a = {}; }
    if (!a.enListen && !a.translate && !a.arListen) continue;
    var enP = { text: (a.enListen && a.enListen.text) || '' };
    var trP = { text: (a.translate && a.translate.text) || '', ref_ar: (a.translate && a.translate.ref) || '' };
    var arP = { text: (a.arListen && a.arListen.text) || '' };
    var g = aiGrade_(enP, (a.enListen && a.enListen.answer) || '', trP, (a.translate && a.translate.answer) || '', arP, (a.arListen && a.arListen.answer) || '');
    if (!g) continue;
    var total = Math.round(((Number(g.enListen) || 0) + (Number(g.translate) || 0) + (Number(g.arListen) || 0)) / 3);
    var rowNum = i + 2;
    sh.getRange(rowNum, 3).setValue(Number(g.enListen) || 0);
    sh.getRange(rowNum, 5).setValue(Number(g.translate) || 0);
    sh.getRange(rowNum, 7).setValue(Number(g.arListen) || 0);
    sh.getRange(rowNum, 8).setValue(total);
    sh.getRange(rowNum, 9).setValue(g.feedback || 'ai');
    updated++;
  }
  bustCache_();
...
// أي تعديل يدوي في Google Sheet (حذف/تعديل طالب…) يُبطل الكاش فورًا ليظهر في البرنامج
function onEdit(e) {
...
function bustCache_() {
  try { CacheService.getScriptCache().remove('bootstrap'); } catch (e) {}
  try { PropertiesService.getScriptProperties().setProperty('dataToken', String(Date.now()) + Math.random()); } catch (e) {}
}
...
function exportResultsSheet_(req) {
  ...
  var token = p.getProperty('dataToken') || '0';
  if (!req.force && token === p.getProperty('builtToken')) return { ok: true, skipped: true };
  var count = buildResultsSheet();
  p.setProperty('builtToken', token);
