...
function importStudents_(req) {
  ...
  rows.forEach(function (s) {
    ...
    else if (seenNew[nid] != null) { toAppend[seenNew[nid]] = row; }   // تكرار داخل الملف
    else { seenNew[nid] = toAppend.length; toAppend.push(row); added++; }
  });
  if (toAppend.length) {                        // كتابة كل الجديد دفعة واحدة
...
function resetLangExam_(req) {
  requireAdmin_(req);
  var idx = findRowIndex_('LangResults', 'nationalId', req.nationalId);
  if (idx === -1) return { ok: true, note: 'لا توجد نتيجة سابقة' };
  sheet_('LangResults').deleteRow(idx);
  return { ok: true };
}
...
function computeResult_(nationalId, cfg, ctx) {
  cfg = cfg || getConfig_();
  if (!ctx) ctx = { interviews: readAll_('Interviews'), unitGrades: readAll_('UnitGrades'), lang: readAll_('LangResults') };
  var indicatorMax = Number(cfg.indicatorMax) || 10;

  // مقابلة: متوسط تقييمات المقيّمين، محوّل إلى نسبة مئوية
  var ivs = ctx.interviews.filter(function (x) { return String(x.nationalId) === String(nationalId); });
  var interviewPct = null;
  if (ivs.length) {
    var sum = 0;
    ivs.forEach(function (v) { sum += (Number(v.avg) / indicatorMax) * 100; });
    interviewPct = sum / ivs.length;
  }
...
function loadCtx_() {
  return { interviews: readAll_('Interviews'), unitGrades: readAll_('UnitGrades'), lang: readAll_('LangResults') };
}
...
function onOpen() {
  try {
    SpreadsheetApp.getUi().createMenu('منظومة القبول')
      ...
      .addItem('تحديث كشف النتائج', 'buildResultsSheet')
      .addToUi();
  } catch (e) {}
}
